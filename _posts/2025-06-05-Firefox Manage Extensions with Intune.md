---
title: "Mozilla Firefox Extensions Settings"
last_modified_at: 2025-06-5T13:33:00+02:00
categories:
  - Blog
tags:
  - Intune
...
If you want to allow, force or block extensions on Firefox through intune you need to do this through a custom configuration profile while ingesting the Firefox ADMX.
In this blogpost I'll show you how you can do this.

**Getting the Firefox ADMX**

1. Go to [https://github.com/mozilla/policy-templates/releases](https://github.com/mozilla/policy-templates/releases) and get the "Policy_Templates_Vx_xx.zip" file
2. Unzip this file

**Uploading the ADMX in Intune**
...
**Mozilla ADMX Import**
Click on "select an .admx file" and find the mozilla.admx inside the following folder: policy_templates_vx_xx\windows of the extracted Mozilla Firefox ADMX .zip file
Click on "Select an .adml file" and select the language folder you want (In my case en-US) and find mozilla.adml in this folder

**Firefox ADMX Import**

1. Click on Import once again
2. Click on "select an .admx file" and find the firefox.admx inside the following folder: policy_templates_vx_xx\windows of the extracted Mozilla Firefox ADMX .zip file
3. Click on "Select an .adml file" and select the language folder you want (In my case en-US) and find firefox.adml in this folder.

**Using custom policy to import and set the extension settings**

1. Go to Intune
2. Go to Devices -> Configuration
3. Create a new policy -> Platform -> Windows 10 and later
4. Profile Type -> Templates
5. Click on "Custom"
6. Input a name and description -> Click Next
...
<policyDefinitions revision="6.11" schemaVersion="1.0">
  ...
  <resources minRequiredRevision="6.11"/>
  <supportedOn>
    <definitions>
...
4. OMA-URI -> ./Device/Vendor/MSFT/Policy/Config/Firefox~Policy~firefox~Extensions/ExtensionSettings
5. Data Type -> String
6. You can now use the folowing example. This will force install ublock, allow 1password and block everything else:

```json
{
  "ublock-origin@contoso.com": {
    "installation_mode": "force_installed",
    "install_url": "<https://addons.mozilla.org/firefox/downloads/latest/ublock-origin/addon-1865923-latest.xpi>",
    "updates_disabled": false
  },
  "1password-x@example.com": {
...
1. You can now push this to your users.

**How to find the extension ID/Name?**

1. Open Firefox and type about:memory in the address bar, then press Enter.
2. Once the page loads, locate the Show memory reports section and click the "Measure" button.
3. After the memory report generates, find the Main Process section on the page.
4. Scroll down within the Main Process section until you reach Other Measurements.
5. In the Other Measurements area, you'll see a list of your installed (active) extensions. Each extension will display its name along with its unique ID in the format: baseURL=moz-extension://[random-id-string].